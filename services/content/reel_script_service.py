...
class ReelScriptService:
    """Generates and persists reel script drafts (draft-only in MVP).

    Phase 10 enhancements:
    - Injects PersonaContext for voice consistency.
    - Injects AntiRepeatContext to prevent repeated hooks.
    - Reel schema now includes hook_0_3s for the critical first-3-second opener.
    """
    ...
    def generate_reel_draft(
        ...
        *,
        ...
        payload = self._runner.run_json(
            db=db,
            prompt_ref=prompt_ref_for_language("reel_writer", language),
            schema=ReelDraftPayload,
            input_payload=input_payload,
            metadata={"kind": "draft_reel", "day": day.isoformat(), "brand_profile_id": brand_profile_id},
        )
